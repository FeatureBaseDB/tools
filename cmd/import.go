...
Agent num has no effect`,
	...
	flags.Int64Var(&importer.BaseRowID, "base-row-id", 0, "Rows being set will all be greater than this.")
	flags.Int64Var(&importer.MaxRowID, "max-row-id", 1000, "Rows being set will all be <= this.")
	flags.Int64Var(&importer.BaseColumnID, "base-column-id", 0, "Columns being set will all be greater than this.")
	flags.Int64Var(&importer.MaxColumnID, "max-column-id", 1000, "Maximum column id to generate.")
	flags.BoolVar(&importer.RandomRowOrder, "random-row-order", false, "If this option is set, the import file will not be sorted by row id.")
	flags.Int64Var(&importer.MinBitsPerMap, "min-bits-per-map", 0, "Minimum number of bits set per row.")
	flags.Int64Var(&importer.MaxBitsPerMap, "max-bits-per-map", 10, "Maximum number of bits set per row.")
